...
const SearchBankBranchesDashboard = props => {
  ...
  const handleSort = sortOrder => {
    ...
    if (sortOrder == 'ASC') {
      sortedBanks = sortedBanks.sort((bankA, bankB) =>
        bankA.bankName > bankB.bankName
          ? 1
          : bankB.bankName > bankA.bankName
            ? -1
            : 0
      )
      ...
      sortedBanks = sortedBanks.sort((bankB, bankA) =>
        bankA.bankName > bankB.bankName
          ? 1
          : bankB.bankName > bankA.bankName
            ? -1
    ...
    const filteredBanks = allBanks.filter(
      bank =>
        ...
        bank.ifscCode
          ?.toUpperCase()
          .includes(event.target.value?.toUpperCase()) ||
        bank.micrCode.toUpperCase().includes(event.target.value?.toUpperCase())
    )
  ...
  const getBankBranch = () => {
    setBlocking(true)
    apicaller('get', `${BASEURL}/bankBranch/by`)
      .then(res => {
        setBlocking(false)
        if (res.status === 200) {
      ...
      .catch(err => {
        setBlocking(false)
        console.log('getBankBranch err', err)
      })
  }
