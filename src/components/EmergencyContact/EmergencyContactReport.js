...
const EmergencyContactReport = (props) => {
  const { user } = props;
  ...
  const [paginationEmergencyContacts, setPaginationEmergencyContacts] =
    useState([]);
  ...
  const handleFilter = (type, depts, locs) => {
    let departments;
    let location;
    let obj = {};

    if (depts?.length > 0) {
      departments = depts.map((a) => a.id);
    }
    if (locs?.length > 0) {
      location = locs.map((a) => a.locationId);
    }

    obj = {
      department: departments,
      location: location,
      reportType: 'EmergencyContact'
    };
    if (isPrimary) {
      obj.isPrimary = true;
    }
        ...
        if (res.status === 200) {
          ...
          if (type == 'excel') {
            ...
            if (res.data.length > 0) {
              ...
              setAllEmergencyContacts(res.data);
              setPaginationEmergencyContacts(res.data);
            } else {
              setEmergencyContacts(res.data);
              setAllEmergencyContacts(res.data);
              setPaginationEmergencyContacts(res.data);
  ...
  const handleSort = (sortOrder) => {
    let sortedEmergencyContacts = JSON.parse(JSON.stringify(emergencyContacts));
    if (sortOrder == 'ASC') {
      sortedEmergencyContacts = sortedEmergencyContacts.sort((empA, empB) =>
        empA.emergencyContactName > empB.emergencyContactName
          ? 1
          : empB.emergencyContactName > empA.emergencyContactName
            ? -1
            : 0
      );
      setEmergencyContacts(sortedEmergencyContacts);
      setPaginationEmergencyContacts(sortedEmergencyContacts);
    } else {
      sortedEmergencyContacts = sortedEmergencyContacts.sort((empB, empA) =>
        empA.emergencyContactName > empB.emergencyContactName
          ? 1
          : empB.emergencyContactName > empA.emergencyContactName
            ? -1
  ...
  const handleSearch = (event) => {
    const filteredEmergencyContacts = allEmergencyContacts.filter((obj) =>
      JSON.stringify(obj)
        .toLowerCase()
    ...
    setEmergencyContacts(filteredEmergencyContacts);
    setPaginationEmergencyContacts(filteredEmergencyContacts);
  };
        ...
        <Card className="card-box shadow-none">
          <div className="d-flex flex-column flex-md-row justify-content-between px-4 py-3">
            ...
            <div className="d-flex align-items-center">
              <FontAwesomeIcon
                onClick={() => handleFilter('excel', filteredDepartments, filteredLocations)}
                icon={['far', 'file-excel']}
                  ...
                  <div className="dropdown-menu-xxl overflow-hidden p-0">
                      ...
                      <Grid container spacing={2}>
                        ...
                        <Grid item md={6}>
                          <small
                            ...
                            className="font-weight-bold pb-2 text-uppercase text-primary d-block">
                            Is Primary
                          </small>
                        </Grid>
                        <Grid item md={6}>
                    ...
                    <div className="p-3 text-center bg-secondary">
                      <Button
                        ...
                        onClick={() => {
                          handleFilter('', filteredDepartments, filteredLocations);
                        }}
                        size="small">
                        Search
                      </Button>
              ...
              <div>
                ...
                <Menu
                  ...
                  <div className="dropdown-menu-lg overflow-hidden p-0">
                    ...
                    <List className="nav-neutral-first nav-pills-rounded flex-column p-2">
                      <ListItem
                        ...
                        onClick={(e) => {
                          ...
                          setPage(1)
                          setPaginationEmergencyContacts(emergencyContacts);
                          handleClose2();
                        }}>
                    ...
                    <div className="divider" />
                    <div className="font-weight-bold px-4 pt-4">Order(By Emergency Contact Name)</div>
                    <List className="nav-neutral-first nav-pills-rounded flex-column p-2">
                      <ListItem
                        button
                        href="#/"
          ...
          <div className="p-4">
            <div className="text-center my-4">
              <span className="display-4 mb-1 font-size-md font-weight-bold pb-4 text-capitalize">
                Employee Emergency Contact Details Report
              </span>
            </div>
            ...
            <div className="table-responsive-md">
              <TableContainer>
                <Table className="table table-alternate-spaced mb-0">
                  ...
                  {paginationEmergencyContacts.length > 0 ? (
                    <>
                      <tbody>
                        {paginationEmergencyContacts
                          .slice(
                            page * recordsPerPage > emergencyContacts.length
                              ? page === 0
                                ? 0
                              ...
                              <tr>
                                ...
                                <td>
                                  <div className="d-flex align-items-center">
                                    <div
                                      style={tableData}
                                      title={item.emergencyContactName}>
                                ...
                                <td>
                                  <div className="d-flex align-items-center">
                                    <div
                                      style={tableData}
                                      title={item.emergencyContactAddress}>
                                ...
                                <td>
                                  <div className="d-flex align-items-center">
                                    <div
                                      style={tableData}
                                      title={item.emergencyContactNumber}>
          ...
          <div className="d-flex align-items-center justify-content-center pt-3 mb-5">
            <Pagination
              ...
              count={Math.ceil(emergencyContacts.length / recordsPerPage)}
              variant="outlined"
              shape="rounded"
              selected={true}
              page={page}
