...
const EmployeeNameHistory = (props) => {
  const {
  ...
  useEffect(() => {
    if (create != null && !create) {
      setFirstName(selectedEmployeeInfoHistory.historyObject.firstName);
      setMiddleName(selectedEmployeeInfoHistory.historyObject.middleName);
      setLastName(selectedEmployeeInfoHistory.historyObject.lastName);
      setEffectiveDate(
  ...
  const save = () => {
    ...
    if (create) {
      ...
      if (
        firstName == selectedEmployeeInfoHistory.historyObject.firstName &&
        middleName == selectedEmployeeInfoHistory.historyObject.middleName &&
        lastName == selectedEmployeeInfoHistory.historyObject.lastName &&
    ...
    if (isValid) {
      if (create) {
        ...
        formData.append('documentType', 'EmployeeName');
        apicaller('post', `${BASEURL}/storage/uploadFile`, formData)
          .then((res) => {
            if (res.status === 200) {
              nameHistoryObj.historyObject.file = res.data;
              saveHistoryObject(nameHistoryObj);
            }
          })
          .catch((err) => {
        ...
        if (files.length > 0) {
          ...
          nameHistoryObj.historyObject.file = savedFileDetails;
          saveHistoryObject(nameHistoryObj);
        }
      }
    } else {
  ...
  const saveHistoryObject = (nameHistoryObj) => {
    if (create) {
      apicaller('post', `${BASEURL}/employeeInfoHistory/save`, nameHistoryObj)
        .then((res) => {
          if (res.status === 200) {
            setEmployeeInfoHistoryDetails([
              null,
            ...
            setState({
              open: true,
              message: 'Employee Name Created Sucessfully',
        ...
        .catch((err) => {
          ...
          setState({
            open: true,
            message: 'Error occured while creating Employee Name',
            toastrStyle: 'toastr-warning',
            vertical: 'top',
            horizontal: 'right'
          });
      ...
      nameHistoryObj.uuid = selectedEmployeeInfoHistory.uuid;
      apicaller('put', `${BASEURL}/employeeInfoHistory/update`, nameHistoryObj)
        .then((res) => {
          if (res.status === 200) {
            selectedEmployeeInfoHistory.historyObject.firstName = firstName;
            selectedEmployeeInfoHistory.historyObject.middleName = middleName;
            selectedEmployeeInfoHistory.historyObject.lastName = lastName;
            selectedEmployeeInfoHistory.effectiveDate = effectiveDate;
            selectedEmployeeInfoHistory.reason = reason;
            selectedEmployeeInfoHistory.historyObject.file =
              nameHistoryObj.historyObject.file;
            setState({
              open: true,
              message: 'Employee Name Corrected Sucessfully',
              toastrStyle: 'toastr-success',
              vertical: 'top',
              horizontal: 'right'
            });
        ...
        .catch((err) => {
          ...
          setState({
            open: true,
            message: 'Error occured while Correcting Employee Name',
            toastrStyle: 'toastr-warning',
            vertical: 'top',
            horizontal: 'right'
          });
    ...
    apicaller(
      ...
      .then((res) => {
        if (res.status === 200) {
          ...
          setState({
            open: true,
            message: 'Employee Name History Deleted Sucessfully',
            toastrStyle: 'toastr-success',
            vertical: 'top',
            horizontal: 'right'
          });
          ...
          <div>
            ...
            <div className="float-left">
              <Button className="btn-primary mb-2 m-2" onClick={(e) => save(e)}>
                {create ? 'Add New Name' : 'Correct History'}
              </Button>
              {!(
                create ||
                savedEmployeeInfoHistoryDetails[
