...
const SearchJobGradeDashboard = props => {
  ...
  const handleFilter = status => {
    if (status > 0) {
      ...
      const filteredGrades = allGrades.filter(
        grade => grade.status == statusLabel
      )
      setGrades(filteredGrades)
      setPaginationGrade(filteredGrades)
    } else {
      setGrades(allGrades)
      setPaginationGrade(allGrades)
    }
  }
  const handleSort = sortOrder => {
    let sortedGrades = JSON.parse(JSON.stringify(grades))
    if (sortOrder == 'ASC') {
      sortedGrades = sortedGrades.sort((gradeA, gradeB) =>
        gradeA.gradeName > gradeB.gradeName
          ? 1
          : gradeB.gradeName > gradeA.gradeName
          ? -1
          : 0
      )
      setGrades(sortedGrades)
      setPaginationGrade(sortedGrades)
    } else {
      sortedGrades = sortedGrades.sort((gradeB, gradeA) =>
        gradeA.gradeName > gradeB.gradeName
          ? 1
          : gradeB.gradeName > gradeA.gradeName
          ? -1
  ...
  const [paginationGrade, setPaginationGrade] = useState([])
  const [grades, setGrades] = useState([])
  const [blocking, setBlocking] = useState(false)
  const [recordsPerPage, setRecordsPerPage] = useState(10)
  const [page, setPage] = useState(1)
  const [formURL, setFormURL] = useState('/createJobGrade')
    ...
    apicaller('get', `${BASEURL}/jobGrade/fetchAll`)
      .then(res => {
        ...
        if (res.status === 200) {
          ...
          setPaginationGrade(res.data?.data)
        }
      })
      .catch(err => {
        setBlocking(false)
            ...
            <div className='p-4'>
              <div className='table-responsive-md'>
                <Table className='table table-alternate-spaced mb-0'>
                  ...
                  <tbody>
                    {paginationGrade
                      .slice(
                        page * recordsPerPage > grades.length
                          ? page === 0
                            ? 0
              ...
              <div className='d-flex align-items-center justify-content-center pt-3 mb-5'>
                <Pagination
                  ...
                  count={Math.ceil(grades.length / recordsPerPage)}
                  variant='outlined'
                  shape='rounded'
                  selected={true}
                  page={page}
