...
const SearchLetterDashboard = props => {
  ...
  const handleFilter = status => {
    if (status > 0) {
      ...
      setPaginationLetter(filteredLetters)
    } else {
      handleClose()
      setLetters(allLetters)
      setPaginationLetter(allLetters)
    }
  }
  const handleSort = sortOrder => {
    let sortedLetters = JSON.parse(JSON.stringify(letters))
    if (sortOrder == 'ASC') {
      sortedLetters = sortedLetters.sort((letterA, letterB) =>
        letterA.name > letterB.name ? 1 : letterB.name > letterA.name ? -1 : 0
      )
      setLetters(sortedLetters)
      setPaginationLetter(sortedLetters)
    } else {
      sortedLetters = sortedLetters.sort((letterB, letterA) =>
        letterA.name > letterB.name ? 1 : letterB.name > letterA.name ? -1 : 0
      )
      setLetters(sortedLetters)
      setPaginationLetter(sortedLetters)
  ...
  const [paginationLetter, setPaginationLetter] = useState([])
  const [letters, setLetters] = useState([])
  const [blocking, setBlocking] = useState(false)
  const [recordsPerPage, setRecordsPerPage] = useState(10)
  const [page, setPage] = useState(1)
  const [formURL, setFormURL] = useState('/createLetters')
            ...
            <div className='p-4'>
              <div className='table-responsive-md'>
                <Table className='table table-alternate-spaced mb-0'>
                  <tbody>
                    {paginationLetter
                      .slice(
                        page * recordsPerPage > letters.length
                          ? page === 0
                            ? 0
              ...
              <div className='d-flex align-items-center justify-content-center pt-3 mb-5'>
                <Pagination
                  ...
                  count={Math.ceil(letters.length / recordsPerPage)}
                  variant='outlined'
                  shape='rounded'
                  selected={true}
                  page={page}
