...
const UploadBeneficiary = props => {
  ...
  const [paginatedBeneficiary, setPaginatedBeneficiary] = useState([])
  ...
  useEffect(() => {
    getBeneficiaryCSVHeader()
  }, [])

  const [csvHeader, setCSVHeader] = useState()

  const getBeneficiaryCSVHeader = () => {
    apicaller(
      'get',
      `${BASEURL}/employeeDependantOrBeneficiary/beneficiaryCSVHeader`
    )
      .then(res => {
        if (res.status === 200) {
  ...
  const saveAll = e => {
    ...
    setBlocking(true)
    apicaller('post', `${BASEURL}/employeeDependantOrBeneficiary/saveAllBeneficiaries`, fileData)
      .then(res => {
        setBlocking(false)
        if (res.status === 200) {
        }
  ...
  const {
    ...
    onDrop: acceptedFiles => {
      acceptedFiles.forEach(async file => {
        ...
        setPaginatedBeneficiary(filteredArray)
        setBeneficiaryData(filteredArray)
      })
  ...
  const csvLink = {
    filename: 'Beneficiary_Bulk_Upload_Template.csv',
    headers: csvHeader,
    data: []
  }
  ...
  const validateUploadingData = () => {
    ...
    beneficiaryData.forEach((item, currentItemIndex) => {
      let isValid = true
      item.errors = []
      ...
      if (item.Beneficiary_Type == null || item.Beneficiary_Type.length == 0) {
        isValid = false
        item.errors.push('Beneficiary Type can not be empty.\n')
      } else if (
        item.Beneficiary_Type.toLowerCase() !== 'charity' &&
        item.Beneficiary_Type.toLowerCase() !== 'estate' &&
        item.Beneficiary_Type.toLowerCase() !== 'organization' &&
        item.Beneficiary_Type.toLowerCase() !== 'person' &&
        item.Beneficiary_Type.toLowerCase() !== 'temple' &&
        item.Beneficiary_Type.toLowerCase() !== 'trust' &&
        item.Beneficiary_Type.toLowerCase() !== 'other'
      ) {
        isValid = false
        item.errors.push(
          'Beneficiary Type should be Charity, Estate, Organization, Person, Temple, Trust, or Other. \n'
        )
      }
      ...
      if (item.Name == null || item.Name.length == 0) {
        isValid = false
        item.errors.push('Name can not be empty.\n')
      }
      ...
      if (
        item.Is_Disabled == null ||
        item.Is_Disabled?.toLowerCase() != ('true') && item.Is_Disabled?.toLowerCase() != ('false')
      ) {
        isValid = false
        item.errors.push('Is Disabled should be true or false. \n')
      }

      if (
        item.Is_Student == null ||
        item.Is_Student?.toLowerCase() != ('true') && item.Is_Student?.toLowerCase() != ('false')
      ) {
        isValid = false
        item.errors.push('Is Student should be true or false. \n')
      }
                ...
                <>
                        ...
                        <List className='nav-neutral-first nav-pills-rounded flex-column p-2'>
                          <ListItem
                            ...
                            onClick={e => {
                              ...
                              setPage(1)
                              setPaginatedBeneficiary(beneficiaryData)
                            }}>
                  ...
                  <div className='p-4'>
                    <div
                      ...
                      <Table className='table table-alternate-spaced mb-0'>
                        ...
                        <tbody>
                          {paginatedBeneficiary
                            .slice(
                              page * recordsPerPage > beneficiaryData.length
                                ? page === 0
                                  ? 0
                              ...
                              <>
                                <tr>
                                  ...
                                  <td>
                                    <div className='d-flex align-items-center'>
                                      <div>
                                        <label className='' title={item.Name}>
                                          {item.Name}
                                        </label>
                                      </div>
                                    </div>
                  ...
                  <div className='d-flex align-items-center justify-content-center pt-3 mb-5'>
                    <Pagination
                      ...
                      count={Math.ceil(beneficiaryData.length / recordsPerPage)}
                      variant='outlined'
                      shape='rounded'
                      selected={true}
                      page={page}
