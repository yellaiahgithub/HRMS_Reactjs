...
const UploadJobBand = (props) => {
    ...
    const [isAutoGeneratedBandId, setisAutoGeneratedBandId] =
        useState(false);
    const [files, setFiles] = useState([]);
    const [paginationUploadJobBandData, setPaginationUploadJobBandData] =
        useState([]);
    const [uploadJobBandData, setUploadJobBandData] = useState([]);
    ...
    const getJobBandCSVHeader = () => {
        apicaller('get', `${BASEURL}/jobBand/CSVHeader`)
            .then((res) => {
                if (res.status === 200) {
            ...
            .catch((err) => {
                console.log('get Job Band err', err);
            });
    };
    ...
    const {
        ...
        onDrop: (acceptedFiles) => {
            acceptedFiles.forEach(async (file) => {
                ...
                setUploadJobBandData(result.data);
                setPaginationUploadJobBandData(result.data);
            });
        }
    });
                ...
                <Box textAlign="right">
                    <Button
                        onClick={(e) => {
                            ...
                            setPaginationUploadJobBandData([]);
                            setUploadJobBandData([]);
                        }}
    ...
    const validateJobBand = () => {
        let dataWithoutErrors = [];
        let dataWithErrors = [];
        setIsValidated(true)
        uploadJobBandData.forEach((item) => {
            let isValid = true;
            item.errors = [];
            if (!isAutoGeneratedBandId) {
                if (item.Band_Id == null || item.Band_Id.length == 0) {
                    isValid = false;
                    item.errors.push('Band Id can not be empty.\n');
                } else {
                    const duplicateJobBandWithId = uploadJobBandData.find(
                        (jBand) => jBand.id === item.Band_Id
                    );
                    if (duplicateJobBandWithId) {
                        ...
                        item.errors.push(
                            "Duplicate Band Id '" +
                            item.Band_Id +
                            "' .Another record with Band name '" +
                            duplicateJobBandWithId?.Band_Name +
                            "' also has same BandId in the uploaded file " +
                            '\n'
                        );
                    }
                }
            }
            if (item.Band_Name == null || item.Band_Name.length == 0) {
                item.errors.push("Band Name can not be empty.\n");
            }
            if (
        ...
        setUploadJobBandData(data);
        setPaginationUploadJobBandData(data);
        setState({
            open: true,
            message:
    ...
    const saveAll = (e) => {
        ...
        const fileData = {
            ...
            data: modifiedData,
            isAutoGeneratedBandId: isAutoGeneratedBandId
        };
        ...
        apicaller('post', `${BASEURL}/jobBand/saveAll`, fileData)
            .then((res) => {
                if (res.status === 200) {
                }
            })
            .catch((err) => {
                ...
                console.log('Upload JobBand err', err);
            });
    };
    return (
        <Card>
            {!hideCodes ? (
                <>
                    <Grid container spacing={0}>
                        <Grid item xs={10} md={10} lg={10} xl={11} className="mx-auto">
                            <Grid item container spacing={2} direction="row" className=' mt-5'>
                                <Grid item md={12} className="font-size-sm d-flex align-items-center ">
                                    ...
                                    <CSVLink
                                        {...{
                                            filename: 'Load_JobBand_Template.csv',
                                            headers: CSVHeader,
                                            data: []
                                        }}
                                        style={{
                ...
                <>
                    {paginationUploadJobBandData.length > 0 && (
                        <>
                            <Box textAlign="right">
                                <Button
                                    className="btn-primary mb-2 mr-3"
                                    onClick={(e) => validateJobBand()}>
                                    Validate
                                </Button>
                            </Box>
                            <div className="p-4">
                                    ...
                                    <Table className="table table-alternate-spaced mb-0">
                                        <thead>
                                            <tr>
                                                ...
                                                {/* <th
                          style={{ width: 'auto' }}
                          className="font-size-lg font-weight-bold pb-4 text-capitalize text-dark"
                          scope="col">
                          Effective Status
                        </th> */}
                                                <th
                                        ...
                                        <tbody>
                                            {paginationUploadJobBandData
                                                .slice(
                                                    page * recordsPerPage > uploadJobBandData.length
                                                        ? page === 0
                                                            ? 0
                                                    ...
                                                    <>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div className="d-flex align-items-center">
                                                                    <div>
                                                                        <label title={item['Effective_Date(DD/MM/YYYY)']}>
                                ...
                                <div className="d-flex align-items-center">
                                  <div>
                                    <label title={item.Effective_Status}>
                            ...
                            <div className="d-flex align-items-center justify-content-center pt-3 mb-5">
                                <Pagination
                                    ...
                                    count={Math.ceil(
                                        uploadJobBandData.length / recordsPerPage
                                    )}
                                    variant="outlined"
                                    shape="rounded"
                                    selected={true}
