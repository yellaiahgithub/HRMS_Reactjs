...
const UploadNationalID = props => {
  ...
  const [paginatedNationalID, setPaginatedNationalIDs] = useState([])
  ...
  useEffect(() => {
    getNationalIDCSVHeader()
  }, [])

  const [csvHeader, setCSVHeader] = useState()

  const getNationalIDCSVHeader = () => {
    apicaller('get', `${BASEURL}/nationalId/CSVHeader`)
      .then(res => {
        if (res.status === 200) {
  ...
  const saveAll = e => {
    ...
    setBlocking(true)
    apicaller('post', `${BASEURL}/nationalID/saveAll`, allFilesData)
      .then(res => {
        setBlocking(false)
        if (res.status === 200) {
        }
  ...
  const {
    ...
    onDrop: acceptedFiles => {
      acceptedFiles.forEach(async file => {
        ...
        setPaginatedNationalIDs(filteredArray)
        setNationalIDData(filteredArray)
      })
  ...
  const csvLink = {
    filename: 'NationalId_Bulk_Upload_Template.csv',
    headers: csvHeader,
    data: []
  }
  ...
  const validateUploadingData = () => {
    let modifiedData = []
    let data = []
    nationalIDData.forEach(item => {
      const data = {
        employeeUUID: item.EmployeeId,
        isPrimary: item['Is_Primary(true/false)'] == 'TRUE' ? true : false,
        name: item.Name_As_Per_Document,
        identificationType: item.Identification_Type,
        Identification: item.Identification_Number,
        isExpiry: item['Is_Expiry(true/false)'] == 'TRUE' ? true : false,
        expiry: item['Expiry_Date(DD/MM/YYYY)'],
    ...
    setBlocking(true)
    apicaller('post', `${BASEURL}/nationalId/saveAll`, fileData)
      .then(res => {
        if (res.status === 200) {
          setBlocking(false)
          ...
          setState({
            ...
            toastrStyle:
              res?.data?.errorCount > 0 ? 'toastr-warning' : 'toastr-success',
            vertical: 'top',
            horizontal: 'right'
          })

          if (res?.data?.errorData) {
            for (let i = 0; i < nationalIDData.length; i++) {
              nationalIDData[i]['errors'] = []
              if (res?.data?.errorData[i]?.errors) {
                nationalIDData[i].errors.push(res?.data?.errorData[i]?.errors)
              }
              data.push(nationalIDData[i])
            }

            setNationalIDData(data)
          }
        }
      })
      .catch(err => {
                ...
                <>
                        ...
                        <List className='nav-neutral-first nav-pills-rounded flex-column p-2'>
                          <ListItem
                            ...
                            onClick={e => {
                              ...
                              setPage(1)
                              setPaginatedNationalIDs(nationalIDData)
                            }}>
                    ...
                    <div
                      ...
                      <Table className='table table-alternate-spaced mb-0'>
                        ...
                        <tbody>
                          {paginatedNationalID
                            .slice(
                              page * recordsPerPage > nationalIDData.length
                                ? page === 0
                                  ? 0
                                  ...
                                  <td>
                                    <div className='d-flex align-items-center'>
                                      <div>
                                        <label
                                          className=''
                                          title={item.Identification_Type}>
                                  ...
                                  <td>
                                    <div className='d-flex align-items-center'>
                                      <div>
                                        <label
                                          className=''
                                          title={item.Identification_Number}>
                                  ...
                                  <td>
                                    <div className='d-flex align-items-center'>
                                      <div>
                                        <label
                                          className=''
                                          title={item.Name_As_Per_Document}>
                                          {item.Name_As_Per_Document}
                                        </label>
                                      </div>
                                    </div>
                                  </td>
                                  <td>
                                    <div className='d-flex align-items-center'>
                                      <div>
                                        <label
                                          className=''
                                          title={item['Is_Expiry(true/false)'] == 'TRUE' ? 'true' : 'false'}>
                                          {item['Is_Expiry(true/false)'] == 'TRUE' ? 'true' : 'false'}
                                        </label>
                                      </div>
                                    </div>
                                  </td>
                                    ...
                                    <div className='d-flex align-items-center'>
                                      <div>
                                        <label
                                          className=''
                                          title={
                                            item['Is_Primary(true/false)'] == 'TRUE' ? 'true' : 'false'}>
                                          {item['Is_Primary(true/false)'] == 'TRUE' ? 'true' : 'false'}
                                        </label>
                                      </div>
                                    </div>
                  ...
                  <div className='d-flex align-items-center justify-content-center pt-3 mb-5'>
                    <Pagination
                      ...
                      count={Math.ceil(nationalIDData.length / recordsPerPage)}
                      variant='outlined'
                      shape='rounded'
                      selected={true}
                      page={page}
